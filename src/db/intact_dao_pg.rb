...
#		@conn = DBI.connect(conn_str, dbuser, dbpasswd)
		@conn = PG.connect({:host => dbserver, :port => dbport, :user => dbuser, :dbname => dbname, :password => dbpasswd})
...
# @return [Arr]
	def open_conn (dbserver, dbport=5432, dbname, dbuser, dbpasswd)
		if @conn.nil?
			conn_str = "DBI:Pg:dbname="+dbname+";host=" + dbserver + ";port="+dbport
#			@conn = DBI.connect(conn_str, dbuser, dbpasswd)
			@conn = PG.connect({:host => dbserver, :port => dbport, :user => dbuser, :dbname => 'intact', :password => dbpasswd})
		end
	end
...
#		sth = @conn.execute(@select_qry, uniprot_acc, uniprot_acc, conf_val)
		params_ary = [{:value => uniprot_acc, :format => 0},
									{:value => uniprot_acc, :format => 0},
									{:value => conf_val, :format => 0}]
		sth = @conn.exec(@select_qry, params_ary)
		row_cont = 0
